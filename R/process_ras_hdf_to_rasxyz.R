#' @title process_ras_hdf_to_rasxyz
#' @description process a ras g##.hdf file into a stream oriented and classified xyz format
...
#'  pts <- process_ras_hdf_to_rasxyz(geom_path = ghdf_path,units = "English Units",proj_string = "EPSG:2277",vdat = FALSE,quiet = FALSE)
#'  }
#' }
#' @seealso
...
#'  [glue][glue::glue]
#' @rdname process_ras_hdf_to_rasxyz
#' @export
#' @import magrittr
#' @import data.table
...
#'
process_ras_hdf_to_rasxyz <- function(geom_path,
                                   units,
                                   proj_string,
                                   ...
                                   vdat = FALSE,
                                   quiet = FALSE) {
  # sinew::moga(file.path(getwd(),"R/process_ras_hdf_to_rasxyz.R"),overwrite = TRUE)
  # devtools::document()
  ...
  if (file.exists(proj_string)) {
    if (stringr::str_sub(proj_string, -3, -1) == "gdb") {
      ...
      sf::st_crs(sf_cross_section_lines) = readLines(proj_string)
