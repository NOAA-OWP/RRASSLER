...
refresh_master_files <- function(path_to_ras_dbase,is_verbose = TRUE) {
  ...
  if(cloud) {
    ...
    df_bucket_data <- aws.s3::get_bucket(bucket = "s3://ras-models/", prefix = "models/",max = Inf,region = "us-east-2")
    list_bucket_data <- c()
    ...
    full_accounting <- readr::read_csv(list.files(root_temp, pattern = utils::glob2rx("*RRASSLER_metadata.csv$"), full.names=TRUE, ignore.case=TRUE, recursive=TRUE),
                                       id = "file_name",
                                       col_types = c("nhdplus_comid" = "character","model_name" = "character","units" = "character","crs" = "character","final_name_key" = "character")) %>%
      suppressWarnings()
    full_accounting <- full_accounting %>% subset(select = -c(file_name))
    ...
    if(is_verbose) { message(glue::glue("Writing files")) }
    ...
    xyz_files <- list.files(path_to_ras_dbase, pattern = utils::glob2rx("*RRASSLER_cs_pts.parquet$"), full.names=TRUE, ignore.case=TRUE, recursive=TRUE) %>% sort()
    hull_files <- list.files(path_to_ras_dbase, pattern = utils::glob2rx("*RRASSLER_hull.fgb$"), full.names=TRUE, ignore.case=TRUE, recursive=TRUE) %>% sort()

    full_accounting <- readr::read_csv(list.files(path_to_ras_dbase, pattern = utils::glob2rx("*RRASSLER_metadata.csv$"), full.names=TRUE, ignore.case=TRUE, recursive=TRUE),
                                       id = "file_name",
                                       col_types = c("nhdplus_comid" = "character","model_name" = "character","units" = "character","crs" = "character","final_name_key" = "character")) %>%
      suppressWarnings()
    full_accounting <- full_accounting %>% subset(select = -c(file_name))
