...
disk_ingest_record <- function(in_file = NULL,
  ...
  for (potential_file in prj_files) {
    ...
    if (any(c('PROJCS', 'GEOGCS', 'DATUM', 'PROJECTION') == file_text)) {
      if (is_verbose) { message('found a projection') }
  ...
  for (g_file in g_files) {
    ...
    if (!(cond1 & cond2)) {
      ...
      file.copy(
        c(g_file,paste0(g_file,".hdf"),p_files,f_files,h_files,v_files,prj_files,o_files,r_files,u_files,x_files,rasmap_files),
    ...
    if (isFALSE(extrated_pts[[1]]) |
      ...
      if (sum(stringr::str_detect(inital_scrape_names, current_initial_name)) > 0) {
        if (overwrite) {
          message(glue::glue("Model already processed into _unprocessed {current_initial_name}, overwriting"))
          # List of files to delete
          files_to_delete <- list.files(file.path(path_to_ras_dbase,"models","_unprocessed",current_initial_name,fsep = .Platform$file.sep),full.names = TRUE,ignore.case = TRUE,recursive = TRUE)
          for (old_file in files_to_delete) {
            unlink(old_file)
    ...
    current_nhdplus_comid <- crosswalk_hull_to_hydrofabric_value(hull,extrated_pts[[3]])
    ...
    if (sum(stringr::str_detect(processed_scrape_names, current_final_name_key)) > 0) {
      if (overwrite) {
        message(glue::glue("Model already processed into {current_final_name_key}, overwriting"))
    ...
    file.copy(
      c(g_file, paste0(g_file, ".hdf"), p_files, f_files, h_files, v_files, prj_files, o_files, r_files, rasmap_files),
