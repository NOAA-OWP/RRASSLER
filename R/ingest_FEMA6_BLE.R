...
ingest_FEMA6_BLE <- function(path_to_ras_dbase,
  ...
  if (stringr::str_sub(path_to_ras_dbase, 1, 2) %in% c('s3', 'ht')) {
    if (is.null(opt_local_path)) {
      ...
      top_of_dir <- tempdir()
      clean <- TRUE
      message(glue::glue("non-local execution and no temp dir selected, making temp at:{top_of_dir}"))
    } else {
  ...
  util_unzip(
    file.path(database_path,"_temp","BLE",HUCID,glue::glue("{HUCID}_models.zip"),fsep = .Platform$file.sep),
    is_quiet = is_quiet
  )

  if (is.null(proj_override)) {
    scrape_ble_lib(database_path,HUCID,is_quiet = is_quiet,overwrite = overwrite,files = "s")
