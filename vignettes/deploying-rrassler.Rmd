...
Our last set of sample data is taken directly from the [RAS2FIM](https://github.com/NOAA-OWP/ras2fim) repo from Iowa.  This folder ("ras2fim-sample-dataset") has both the input and output from a RAS2FIM (v1) run, and the models are given to us in "SI Units" and in "[EPSG:26915](https://epsg.io/26915)".

## Ingesting the sample data to a sample catalog

One of the first steps you need to do is chose a location to write your database out to.  We'll use our sample data directory and place these in 
```{r, eval=FALSE}
